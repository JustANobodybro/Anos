...
GUI["6g"].InputBegan:Connect(function(input, gameProcessedEvent)
...
GUI["5e"].InputBegan:Connect(function(input, gameProcessedEvent)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragging = true
        mousePos = input.Position
        framePos = GUI["5e"].Position
    end
end)

GUI["5e"].InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement then
        dragInput = input
    end
end)
...
input.InputChanged:Connect(function(input)
    if input == dragInput and dragging then
        local delta = input.Position - mousePos
        GUI["5e"].Position = UDim2.new(framePos.X.Scale, framePos.X.Offset + delta.X, framePos.Y.Scale, framePos.Y.Offset + delta.Y)
    end
    if input.UserInputState == Enum.UserInputState.End and input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragging = false
    end
